...
def write_outputs(root: pathlib.Path, result: dict[str, Any]) -> None:
    ...
    (results / "internal_validation_results_current.json").write_text(
        json.dumps(result, indent=2, sort_keys=True, allow_nan=False) + "\n",
        encoding="utf-8",
    )
    with (results / "validation_checks_current.tsv").open("w", encoding="utf-8", newline="") as handle:
...
def verify_recorded(root: pathlib.Path, current: dict[str, Any]) -> None:
    recorded_path = root / "results" / "internal_validation_results_recorded.json"

...
1. `AUDIT_CONTRACT.md`: primary reproduction, covariance decomposition,
   ...
   before the corresponding numerical execution.
...
For publication-level interpretation of the historical exploratory contract, see `PUBLIC_INTERPRETATION_NOTE.md`; equality of the full 72-dimensional covariance-null and host-by-anchor interaction subspaces is not claimed without a direct subspace comparison.
...
The acquisition command checks out the version-locked commit in detached-HEAD state
...
creates a new isolated working directory and virtual environment, obtains a fresh upstream
checkout, empties `results/`, executes every runner and test, and verifies both
the scientific values and package integrity:
...
This verifies internal structure, recorded statuses, counts, integrity conditions, and version-locked numerical
...
- Primary: `PASS_WITH_FLAGGED_NUMERICAL_SENSITIVITY`, because the pre-specified
...
- Delivered-result verifier: all 16 named verification checks `PASS`, including the corrected
  delivery ID, TSV schema,
  authorship/CFF, ablation classification, SN-Ia constraint drop,
  leave-one-out match, report generation, no-upstream-bytes, and root package integrity.
...
| `results/leave_one_block_out.tsv` | Pre-specified equation-block removals |
...
| `results/exploratory_variance_component_profile.tsv` | Full pre-specified profile grid |
...
| `results/final_verification_summary.json` | Final named-check PASS summary |

#!/usr/bin/env python3
"""Deterministic Phase 1E manifest and ZIP helpers."""
...
EXCLUDED_DIRECTORY_NAMES = {".git", ".venv", ".pytest_cache", "__pycache__", "build", "dist"}
...
def package_files(project: pathlib.Path) -> list[pathlib.Path]:
    ...
    for path in project.rglob("*"):
        ...
        if not path.is_file() or relative.as_posix() in CHECKSUM_FILES or path.suffix in {".pyc", ".pyo"}:
            continue
        files.append(path)
...
def manifest_rows(project: pathlib.Path) -> list[dict[str, str | int]]:
    return [
        {"path": path.relative_to(project).as_posix(), "bytes": path.stat().st_size, "sha256": sha256_file(path)}
...
def write_manifests(project: pathlib.Path) -> list[dict[str, str | int]]:
    ...
    with (project / "MANIFEST.tsv").open("w", encoding="utf-8", newline="") as handle:
...
def verify_manifests(project: pathlib.Path) -> dict[str, str | int]:
    with (project / "MANIFEST.tsv").open(encoding="utf-8", newline="") as handle:
        reader = csv.DictReader(handle, delimiter="\t")
        if reader.fieldnames != ["path", "bytes", "sha256"]:
            raise RuntimeError("MANIFEST.tsv schema mismatch")
        recorded = [{"path": row["path"], "bytes": int(row["bytes"]), "sha256": row["sha256"]} for row in reader]
    expected = manifest_rows(project)
    if recorded != expected:
        raise RuntimeError("manifest differs from delivered tree")
    expected_sums = "".join(f"{row['sha256']}  {row['path']}\n" for row in expected)
    if (project / "SHA256SUMS.txt").read_text(encoding="utf-8") != expected_sums:
        raise RuntimeError("SHA256SUMS differs from manifest")
    return {"status": "PASS", "manifested_file_count": len(expected)}


def deterministic_archive(project: pathlib.Path, archive: pathlib.Path) -> dict[str, str | int]:
    verify_manifests(project)
    archive.parent.mkdir(parents=True, exist_ok=True)
    paths = sorted(
        package_files(project) + [project / "MANIFEST.tsv", project / "SHA256SUMS.txt"],
        key=lambda path: path.relative_to(project).as_posix(),
    )
    with zipfile.ZipFile(archive, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as handle:
        for path in paths:
            relative = path.relative_to(project).as_posix()
            info = zipfile.ZipInfo(f"{project.name}/{relative}", date_time=(2026, 8, 2, 0, 0, 0))

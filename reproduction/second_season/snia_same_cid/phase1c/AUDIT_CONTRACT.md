...
Contract identifier: `H0DN-SNIA-CONTRAST-COVARIANCE-PHASE1C-20260730-02`
...
### Pre-execution schema hold and contract restart

Contract 01 was frozen and executed only through source, mapping, group, and
covariance-schema checks. It stopped before constructing any Phase 1C
quadratic form because both official 1701-by-1701 covariance files have a
maximum absolute transpose difference of approximately `3.0e-8`, exceeding
the contract-01 bound of `1e-12`. This formatting-level asymmetry in the
STAT+SYS file was already documented in Phase 1B; the corresponding STATONLY
schema value was first read during this pre-execution check.

Contract 01, its freeze record, and its config are preserved as retired
artifacts. `provenance/PREEXECUTION_SCHEMA_HOLD.json` records the failed
gate and confirms that no new contrast chi-square, lower-tail probability,
component diagnostic, or sensitivity classification had been computed.
Contract 02 changes only the official transpose bound from `1e-12` to
`5e-8`, while retaining the already declared averaging transformation
\((C+C^\mathsf T)/2\). All scientific rules, thresholds, baselines, and
other tolerances are unchanged.
...
transpose difference must not exceed `5e-8`; the numerical calculation then
...
| Covariance schemas | dimensions 277 and 1701; finite; official transpose error at most `5e-8` |
